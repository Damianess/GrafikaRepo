...
def draw_polygon(surface, num_points):
    ...
    for i in range(num_points):
        ...
        x = middle[0] + int(radius * math.cos(angle))
        y = middle[1] + int(radius * math.sin(angle))
        a.append((x, y))

    pygame.draw.polygon(surface, CYJANOWY, a, 0)

polygon = pygame.Surface((width, height))
polygon.fill(SZARY)
draw_polygon(polygon, 13) # 13-kąt
...
while run:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            pygame.quit()
            sys.exit()
            #przekształcenia za pomocą pygame.transform
